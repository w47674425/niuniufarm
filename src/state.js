...
export function createState() {
  return {
    ...
    deadHerders: 0,       // 已死亡牧民数（市场旁复活按钮计数，看广告一次复活 1 名）
    lastSave: Date.now()
  };
}
...
// 从棋盘上移除 n 张指定 type 的卡（用于地标建造消耗材料）
// 返回实际移除的数量（可能 < n 如果场上不够）
export function removeTypeN(game, type, n) {
  let removed = 0;
  const cards = allCards(game).filter(c => c.type === type);
