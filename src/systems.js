...
import { DAY_LEN, DAY_FRAC, TICK_MS, CARD_W, CARD_H, MON_SPEED, ENGAGE_DIST, META, PACKS, TASKS, SAVE_KEY, META_KEY, TICKET, MONEY_NAME, foodCapOf, COW_BREEDS } from './config.js';
import { rand, clamp } from './utils.js';
import { mk, makePile, removePile, allCards, countType, removeCardObj, detach, scatter, popCount, markSeen } from './state.js';
import { isFood, pileAction as _pileAction, doAction as _doAction, isDogCard } from './merge.js';
...
// ===================== 昼夜切换 =====================
// 新世界观：夜晚恢复小偷袭击（策划「小偷」表），狗自动迎击（决策4 自动+手动共存）
export function updatePhase(game) {
  ...
  if (newPhase !== st.phase) {
    ...
    spawnMonsters(game);   // 入夜刷怪（策划刷新表）
  }
}

export function onNightStart(game) { toast(game, "🌙 夜幕降临，当心小偷出没！"); }
export function onDayStart(game) { toast(game, "☀️ 天亮了"); clearMonsters(game); }

// ===================== 怪物（策划「小偷」表：夜晚编号=day） =====================
// 0=thief 1=bandit 2=capitalist 3=spy
// 第1晚不来；2-5小偷1；6-10小偷2；11-15小偷2；16-20小偷3；21-25小偷3；26-30大盗1；31-35大盗2；
// 36-40大盗3；41-45大盗3；46-50资本家1；51-55资本家2；56-60资本家3；61-65资本家3；
// 66-70间谍1；71-75间谍2；76-80间谍3；80+间谍3
...
// 夜晚：怪物扑向防御者（被动）+ 牧羊犬自动寻敌（决策4：自动迎击与手动拖拽共存）
export function moveMonsters(game) {
  const st = game.state;
  if (st.phase !== "night") return;
  const dogPiles = st.piles.filter(p => p.cards.some(isDogCard));
  const herderPiles = st.piles.filter(p => p.cards.some(c => c.type === "herder"));
  if (dogPiles.length === 0 && herderPiles.length === 0) return;
  ...
  st.piles.slice().forEach(p => {
    ...
    if (!mon) return;
    if (p.cards.some(c => c.type === "herder" || isDogCard(c))) return; // 同堆已交战
  ...
  // B. 牧羊犬自动寻敌：狗堆主动走向最近怪物，相遇即交战
  if (dogPiles.length > 0) {
    const monPiles = st.piles.filter(p => !p.isPack && p.cards.some(c => META[c.type].cat === "mon"));
    if (monPiles.length > 0) {
      dogPiles.slice().forEach(dp => {
        if (dp.cards.some(c => META[c.type].cat === "mon")) return; // 已在战斗中
        let best = null, bd = 1e9;
        monPiles.forEach(mp => {
          const dx = (mp.x + CARD_W / 2) - (dp.x + CARD_W / 2), dy = (mp.y + CARD_H / 2) - (dp.y + CARD_H / 2);
          const d = dx * dx + dy * dy;
          if (d < bd) { bd = d; best = mp; }
        });
        if (!best) return;
        const dx = (best.x + CARD_W / 2) - (dp.x + CARD_W / 2), dy = (best.y + CARD_H / 2) - (dp.y + CARD_H / 2);
        const dist = Math.sqrt(bd);
        if (dist <= ENGAGE_DIST + 8) {
          const cards = dp.cards.filter(c => isDogCard(c) || c.type === "herder");
          dp.cards = dp.cards.filter(c => !(isDogCard(c) || c.type === "herder"));
          if (dp.cards.length === 0) removePile(game, dp);
          best.cards = best.cards.concat(cards);
          return;
        }
        const ux = dx / dist, uy = dy / dist;
...
export function onDayEnd(game) {
  ...
  st.piles.forEach(p => {
    p.cards.forEach(c => {
      ...
      if (c.type === "sheep" && c.sheepToday) c.sheepToday = 0;   // 羊剪毛配额重置
    });
  });
...
export function loadMeta() {
  try {
    ...
    if (!raw) return { stamps: {}, unlocked: [] };
    const m = JSON.parse(raw);
    return { stamps: m.stamps || {}, unlocked: m.unlocked || [] };
  } catch (e) { return { stamps: {}, unlocked: [] }; }
}
