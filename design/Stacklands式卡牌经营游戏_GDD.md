...
- 科技树阶段：原始部落（牛链起步）→ 工业文明（冶炼厂/农场）→ 飞向宇宙（火箭/科学家）。
  - 起点为牛链而非 Stacklands 浆果丛——用户拍板（2026-08-18）：主体必须围绕牛，见 §7 修订。
...
- **Sources**：卖牛（cattle→money，牛链末端产出的牛拖到卖卡堆，月尾按币值结算——**硬币唯一主源**，用户拍板 2026-08-18）；木材/围栏/银行卡链为存储与辅助，非硬币主源（见 §7 修订）。
- **Sinks**：买卡包（唯一硬 sink）。
- **Edge cases**：
...
- **Sources（食物）**：食物卡包（汉堡）/ 草料链（喂牛）/ 烹饪链——以牛链为中央支柱（用户拍板 2026-08-18），浆果/蓝莓丛仅作辅助安全垫（见 §7 修订）。
...
| 开局启动金 | 25（原 10） | 早期坡度试验假设（信号 B：前 3 月饿死率区间，2026-08-18 登记） | `[PLACEHOLDER·待 playtest]` | 首轮 playtest 按 §8-B 回调 |
...
3. `moonLength`、开局启动金、腐坏计时器需 Monte Carlo + 首轮 playtest 定（卡位上限已由 Monte Carlo 定初值 60，见 §6）。
